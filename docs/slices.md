...
> ### 2026-09-24 — **`src/` IS PORTED TO DESIGN REVISION 6. THE LANGUAGE CONTROL IS ON THE BOARD AND THE EDITOR'S DOOR SAYS `Cha mẹ`.**
>
> The app half of revision 6, against `ui.md` §0D / §9.4 / §9.4a / §9.4b / §9.6 / §10.6,
> `gameplay.md` §0D / §7.1–§7.3 and `acceptance-criteria.md` §0F / §Y / §W6. **Both of the
> owner's real-device findings are closed in the app**; U31 is what closes them with him.
>
> | | State |
> |---|---|
> | `bash scripts/check.sh` | **ALL GREEN.** `npm test` **415/415** (was 390/393 — the 3 were `layout-parity`, by design, saying `src/` was a revision behind) |
> | `src/layout/layout.mjs` | **Revision 6 ported.** `TOP_BAR` 72, `CHROME` 96, `LANG_W`/`DOOR_W`/`DOOR_PAD`/`BAR_PAD`/`BAR_AIR`/`SLOT_GAP`/`TITLE_PT`/`DOOR_LABEL_PT`/`shelfRowW`, the new shelf formula, **F18/F19 as PLAN rules** and **F20/F21/F22 as a constant `LAW`**. Parity is green and now also compares **the rule lists name for name**, so a rule cannot be dropped in transcription |
> | Measured, restated in the tests | **iPhone 17 Plus 430×932: VI 3 pages `[15 ¦ 14 ¦ 6]`, cap 28, 3×5, tile 101 → 99; strip 6 × (58 × 88), glyph 47. EN 1 page, no rail, tile 84 → 83.** 440×956: tile 104 → 102. **The 360×640 floor and the iPad 834×1194 are unchanged.** Two the design's table does not name and the sweep does: **an iPhone SE 3 goes 20 → 16 cells a page** (same 3-page plan), and **a landscape iPhone 17 Plus stops passing F7** — revision 4's F7-vs-P7 deviation shrinks back |
> | The child's control | 72 × 72, board top-left, **no gate, no hold, no multiplication**. Two stacked bars, no glyph and no text, **top = Vietnamese always** — the order is read from `chooserPanels()`, the same function the chooser lays out from, so it is one fact in one place. 18 pt radius, neutral outline, no role bar, not the tile component (Y10). Hidden during the announcement and the reveal (Y8). **One UI tap sound, never speech** — `controller.languageTap()`, driven in Node |
> | It opens the chooser | and **confirming the language he is already in is a free return**: the board stays mounted, the strip, the shelf and the audio are untouched (Y12). Driven in Chromium: `c` `h` in the strip, chooser, confirm Vietnamese, **`c` `h` still in the strip** |
> | The parent door | 65 × 32 pill, `Cha mẹ` / `Parent`, 13 pt `inkSoft`, 1.5 pt `neutralFace`. **The lock is byte-identical** — 1.2 s hold, the spelled-out multiplication, 3 wrong → 30 s, 180 s grace. Driven: a 1.4 s hold opened `Nhập kết quả · tám nhân bốn` |
> | **I2 restated** | a *tap* shows the silent hold hint (`Giữ` / `Hold` + a 1.2 s ring) for 1.6 s, and it is shown **once unprompted** on the first board after a language is committed, guarded by one persisted setting (`holdHintShown`, **Y21**). Driven on a fresh profile: opacity 1 on the first board, 0 after it fades, 0 on the next board, 1 again on a door tap |
> | Parent menu | **Six rows, row 1 `Từ vựng` / `Words` with `thêm, sửa, xoá` / `add, edit, delete`.** `Add a word` is gone from the menu, from the shell's routes and from both string tables. Driven: gate → menu → row 1 → the 47-word list |
> | **A2 restated** | the chooser speaks **`Tiếng Việt` / `English`**. **The clips do not exist** — E23 / Y31–Y34 are the content-engineer's and the owner's ear. **Y33's degradation is built and is the live path today**: `languageNameSource` returns `null` for an absent, malformed or dangling reference, the panel expands silently, and a test asserts *neither shipped manifest carries the clip yet* so the day one does, it says so |
> | A defect a browser found that no test could | **`Cha mẹ` rendered as `Cha …`.** React Native's box model is border-box, so a 65 pt door with a 1.5 pt outline **and** 8 pt of padding gives its label 46 pt against 48.8. F22 says nothing about a border. Fixed by drawing the outline inside the padding, and `test/topbar.test.mjs` now computes that box **from the stylesheet** instead of from the law's two constants |
> | **`test/topbar.test.mjs` restated, and the trap was real** | the revision-5 file read `GATE_DOT` out of the component; the constant is gone, so `Number(undefined)` made its title box **`NaN`**, every `box < worst` was `false`, and **the one test that does the measuring passed having measured nothing**. Proven by arithmetic and by running the committed file against the new bar. It now models the real bar: the title's box is `shelfRowW` (F19), the door's is `DOOR_W − 2·DOOR_PAD` **read from the stylesheet**, F18 over both, and the law's `TITLE_PT` / `DOOR_LABEL_PT` are asserted to **be** the shipped font's measurements |
> | Fault injections | **22, every one exit 1.** Six on the bar (the border clip, the title back at the left edge, a lane claiming flex-grow, a glyph on the control, a sound on the door, `Người lớn`), five on the law **poisoned in the app and the tool together** (`TITLE_PT` 200 → F19, `BAR_PAD` 24 → F18 at 10.2 pt of air, `LANG_W` 64 → F20, `TOP_BAR` 56 → F21, `DOOR_W` 49 → F22; the sweep exits 1 and so does parity), and ten on the wiring (speech on the control, silence on the control, `Add a word` restored, the flag as component state, the free return deleted, the control left up through the reveal, English on top, a throwing clip lookup, the control asking the gate, an unremoved back-handler), plus `GAP_STRIP` 12 → 0 for Y6 |
> | Driven, not asserted | Chromium at **430 × 932** (both languages), **360 × 640** and **834 × 1194**: first launch → chooser → board with the unprompted hint, the control's bars filled/outlined the right way round in each language, the chooser from the board with its *you are here* dot, the free return, a real switch to English, the door tap, the 1.4 s hold, the multiplication, the six-row menu and the word list. **No truncated text and nothing off-screen at any of the three sizes** |
> | **Not verified** | **still never run on a real device.** U28–U33 are revision 6's Tier-5 questions and **U31** — *does `Cha mẹ` in the corner read to the owner's wife as a button, without being told?* — is the one that closes the finding. The two language-name clips are owed before Y34 can be answered |
>
> **A second conflict, measured: Y7 versus F18, by one point.** Y7 asks that *no other
> control's hit rect comes within 12 pt* of either door. The nearest is a shelf slot, and
> `space-between` splits what the law leaves over: at the 360 dp floor that is
> `328 − 72 − 169 − 65 = 22` pt, **11 pt a side** (11.5 on a 390 phone, 13 on the owner's).
> The law never promised 12 a side — **`BAR_AIR` is 12 pt *total*, which F18 spends as two
> 6 pt separations.** Built as the law computes it, asserted at the measured 11, and
> recommended: restate Y7 as ≥ `BAR_AIR / 2` per side, **or** raise `BAR_PAD` 32 → 36 in
> `tools/layout-sweep.mjs`, which buys the twelfth point out of the shelf slot. The second
> changes the Tier-4 artefact and is the game-designer's call, not this port's.
>
> **One conflict reported rather than absorbed: Y14 versus `ui.md` §9.6's figure.** Y14 says
> every tappable thing on the chooser is **≥ 72 pt in its smallest dimension**, including the
> three theme buttons; §9.6's figure says **three 56 pt theme buttons**. Built as the figure
> draws it — 56 pt of ink — with the criterion's target met by the component's existing
> `hitSlop={8}` (56 + 16 = 72). The confirm `▶` is a full 72 pt of ink. If the game-designer
> means 72 pt of *ink* on the chooser's theme buttons, it is one number in `ChooserScreen.js`.

> ### 2026-09-24 — **THE FIRST REAL-DEVICE BUG: THE APP WAS SILENT ON AN iPHONE AFTER THE CHOOSER. FIXED, AND THE FIX IS NOT VERIFIABLE BELOW THE OWNER.**
>
> The owner: *"I can hear sound when I start the game (it said `mèo` when I start choosing
> Vietnamese menu). But after that, forming a word doesn't have any sound."* And the seat
> click — bundled in the binary, nothing to do with pack media — *"No — completely silent."*
>
> | | State |
> |---|---|
> | Cause | `preloadForTable` built **one native player per clip for the whole table**: `createAudioPlayer` **100×** for `vi-seed`, **121×** for `en-seed`, in one synchronous pass. On iOS each is an `AVPlayer` with a live item, a time observer and a KVO subscription; AVFoundation allocates playback pipelines **per process** and refuses past its ceiling. `acquire()` caught the throw and **returned `null` with no signal**, so every later `play()` was a no-op and nothing said so |
> | Why no test caught it | **the criterion itself required it.** `acceptance-criteria.md` N11 said *every clip the constant table can produce is resident before the first tap*. Chromium creates all 157 happily — a browser run is not evidence for this criterion, and never was |
> | The bound | **24 native players, process-wide**, in `src/audio/channels.mjs` (`MAX_PLAYERS`). Derived and measured, not chosen: 7 pinned touch-immediate UI sounds + the 14 tap clips of a `vi-seed` page on the owner's phone plan + the undo's = 22 resident, 2 spare for the announcement. On a one-page tablet 7 + **all 17 live** `vi-seed` tap clips = exactly 24, so **every tile that leads to a word is warm on every target device** |
> | N11, restated | `acceptance-criteria.md` **§0E** and the new N11 / **N11a**. `ui.md` §11.1 carries the correction inline. The worst case is stated: a **cold** clip costs one player construction, which is **not** guaranteed inside N1's 60 ms and is a deliberate deviation |
> | The check that fails | `test/game-controller.test.mjs` — *the pool NEVER exceeds its bound, for either seed pack*, driven through the **real** pool and the **real** warm path. **Poisoned** by restoring the unbounded preload: `vi-seed/tablet: 156 native players after the pack loaded, over the bound of 24` — **exit 1**. Removing the bound alone also exits 1 in `test/audio-channels.test.mjs` |
> | The check the ORCHESTRATOR caught | the first version of that check was **relative** — `live <= MAX_PLAYERS` — so `MAX_PLAYERS = 100000` passed all 19 audio tests. Now guarded **absolutely, against literals the tests own**: `PLAYER_FLOOR <= MAX_PLAYERS <= 32`, and the real warm path holding ≤ 32 players whatever the constant says. Poisoned at **100000** → 2 failures in the channels suite, 1 in the controller suite (`vi-seed/tablet: 39 native players`), **exit 1**; poisoned at **4** (below the floor) → **exit 1**. The failure text tells the next reader to argue with the device, not with the number |
> | Never swallowed again | **N11a.** A construction that throws is counted, remembered, retried once, and **lowers the ceiling** a step (24 → 20 → 16 → 12 → 8, floor 8), so a device stricter than 24 recovers into a quieter app instead of a mute one |
> | The diagnostic the owner reads | **parent menu → About**: `Players: 22/24 · built 31 · failed 0 · silent 0`, in his language. **This is the instrument.** If it still says silent, the numbers say which |
> | `bash scripts/check.sh` | **ALL GREEN.** `npm test` **391/391** (was 385) |
> | **Not verified** | **nobody on this team has an iPhone.** Everything above is Node-driven against the real pool plus a web bundle that builds. The device check is the owner's, and it is the About screen |

> ### 2026-09-24 — **DESIGN REVISION 6 IS ISSUED. THE LANGUAGE SWITCH IS THE CHILD'S; THE EDITOR'S DOOR IS VISIBLE.**
>
> The owner ran the app on a **real iPhone** for the first time and reported two findings:
> he could not find the language switch, and he could not find the editor. One cause — both
> lived behind a 1.2 s hold on a **32 pt dot at 30 % opacity**. **Hiding the gate had also
> hidden the door.** He then overturned the first half of the proposed remedy: *"I think he
> should be able to change the language himself."*
>
> | | State |
> |---|---|
> | `ui.md` / `gameplay.md` / `acceptance-criteria.md` | **Revision 6.** `ui.md` §0D, §9.4/§9.4a/§9.4b, §9.6, §10.6; `gameplay.md` §0D, §7.1, §7.2, §7.3; `acceptance-criteria.md` §0F, **new group §Y (34 criteria)**, §W6. **486 unique criteria, no duplicates** (was 445) |
> | The language control | **The child's.** 72 × 72 at the board's top-left, **no gate, no hold, no multiplication.** Two stacked bars, top = Vietnamese always, filled = the language you are in — the live/flat vocabulary he learns in his first three taps. **No glyph, no letter, no word**, so it can never be mistaken for a 36th character |
> | It opens the **chooser**, not a toggle | He cannot read, so the only way he can know what he is choosing is to **hear** it, and both names live on one screen. A toggle also charges a part-built word for every stray press; **confirming the language he is already in is a free return** — strip, shelf and audio untouched |
> | The parent door | **The lock is unchanged and that is the ruling.** 1.2 s hold → the spelled-out multiplication → 30 s cooldown → 180 s grace, all identical. Only the door changed: a **65 × 32 pill reading `Cha mẹ` / `Parent`**. A word, not an icon — a gear is interesting to a 4-year-old and a word is furniture. It is the only object in the app that **never makes a sound** |
> | **I2 restated** | a tap on the door used to do **nothing**, which is why the owner concluded there was no button. It now shows the hold hint, silently, for 1.6 s — and the hint is shown **once, unprompted**, on the first board after a language is committed, when the person holding the phone is provably an adult |
> | Parent menu | **Six rows, row 1 is `Words — add, edit, delete`.** `Add a word` is folded in: two rows that both led to the editor made neither of them the editor, which is exactly what he asked about |
> | **A2 restated** | the chooser panel speaks **`Tiếng Việt` / `English`**, not `mèo` / `cat`. A sample word identifies a language only to someone who already knows that word *and* has connected it to a language. **Neither clip exists** — handed to the content-engineer as **E23 / Y31–Y34**, and it is **speech**, so the owner must hear them before they ship |
> | **Measured cost** | `TOP_BAR` 56 → 72, `CHROME` 80 → 96 — the price of a 72 pt child control at the unlowered motor floor. **iPhone 17 Plus: VI still 3 pages `[15 ¦ 14 ¦ 6]`, tile 101 → 99; EN still one page, no rail. The 360 × 640 floor: unchanged. The iPad: unchanged.** Shelf slot 32–44 → 29–41. One regression: **360 × 800 Android English goes 1 page → 2.** Served combinations 491,283 → **491,251** |
> | Rejected, with numbers | a 72 pt row of its own (or folding it into the rail) costs **84 pt** and takes the **iPad's Vietnamese board from one page to two**; putting it in the word-strip row drops the 360 dp strip glyph to **27 pt against F4's 34 pt floor** |
> | `tools/layout-sweep.mjs` | **Exit 0.** New rules **F18 / F19** (plan rules) and **F20 / F21 / F22** (constant laws). 34,023,720 layouts + 982,122 page plans |
> | `tools/theme-contrast.mjs` | **Exit 0.** 141 gated checks, 5 new pairs, and **no new colour** — every token pair the two controls create was already measured |
> | Fault injections | **6, every one exit 1 — and TWO WERE MISSED on the first pass.** (1) `DOOR_W` was **derived** from the label, so lengthening the label to `Người lớn` raised the reservation with it and passed, exit 0; `DOOR_W` is a literal now and **F22** bites. (2) **F18 and F19 were written into `RULES`, where a rule cannot be seen to fail** — it only makes a viewport *unserved*, so poisoning `TITLE_PT` to 200 pt deleted every 360 dp phone and still printed `PASS — 0 failing layouts`. That is the **F0 tautology** in a new place. Both are **plan rules** now. The six: F22 (label too long), F18 (`BAR_PAD` 32 → 24, the slop error this revision found), F19 (`TITLE_PT` 200), F20 (`LANG_W` 64), F21 (`TOP_BAR` 56), and the door's label drawn in `neutralFace` → **3.01–3.04:1 against a 4.5 gate, exit 1** |
> | **`npm test` is 390 / 393 and that is BY DESIGN** | the three failures are **all in `test/layout-parity.test.mjs`**: *"the app layout law is identical to the tool"*, *"V4–V8 the page plan and the fixpoint"* and *"gridFor is identical to the tool"*. **`src/layout/layout.mjs` is revision 5 and the tool is revision 6.** The test is doing exactly its job — it is the file that says `src/` is behind the design. The port is three constants and one formula: `TOP_BAR` 72, `CHROME` 96, and `shelf = clamp(floor((CW − 72 − 65 − 32) / 5.4), 0, 44)`. `test/topbar.test.mjs` must also be restated: it measures the title against the **left edge**, and the title is now drawn **under the shelf** |
> | **Not verified** | nothing in revision 6 has been on a device. **U28–U33** are the new Tier-5 questions, and **U31** — *does `Cha mẹ` in the corner read to the owner's wife as a button, without being told?* — is the one that closes the finding that produced this revision |
>
> **Recorded because it was an error and not a judgement call:** the brief this revision was
> written against said the language switch must stay behind the gate because *"a 4-year-old
> must not be able to change the language"*. **That was the orchestrator's assumption, not the
> owner's**, and the owner rejected it. `gameplay.md` §7.1's revision-3 argument is withdrawn,
> not superseded.
...
| # | Slice | State |
|---|---|---|
| **0** | Squad, spike, and approved design | **Revision 6, 2026-09-24, AND `src/` NOW IMPLEMENTS IT** — from the owner's first real-device run: **the language switch is the child's, on the board, with no gate**, and the parent door is a visible word with an unchanged lock (`ui.md` §0D). Previously **revision 5, 2026-09-24.** Rewritten three times from the owner's device testing: r3 one constant table and no ∅ socket, r4 paging on his iPhone 17 Plus, **r5 the board is the standard alphabet and a digraph is spelled letter by letter over an unchanged đánh vần model**. Both design tools exit 0; F0/F12/F15/F17 each seen to fail when poisoned |
...
| **3** | The game plays, both languages, on a real device | **Ported to revision 6 and driven** in Chromium at 430×932 (both languages), 360×640 and 834×1194: the child's 72 pt language control at the board's top-left, the `Cha mẹ` / `Parent` door, the hold hint, the chooser as a free return, a real switch, and the unchanged lock. `npm test` 415/415; 22 fault injections, all exit 1. **Still not run on a real device — U31 is the Tier-5 question that closes the owner's finding.** Previously: **rebuilt for revision 5 and driven** in Chromium at 430×932 (both languages), 360×640 (paged, rail of 3) and 834×1194 (35 cells, no rail): the letter board, the span bar, the marked boundary, whole-strip undo, the superseding tap sound, uppercase English glyphs and the photograph. `npm test` 317/317; 14 fault injections, all caught. **Still not run on a real device — Tier 5 is owed.** Previously: **rebuilt for revision 4 and driven** in Chromium at 430×932, 440×956, 834×1194 and 360×640, both languages, including the language switch and the gate grace. Previously: **rebuilt for revision 2.** Chrome at iPad portrait + landscape, iPhone 393×852 and the 360×640 Android floor, both languages; five words to the album; the gate opens on a 1.2 s hold. **Not yet run on a real device — Tier 5 is owed.** Awaiting independent verification |
...
| **5** | Polish: motion, sound, accessibility | Not started. **Owed to the owner first:** a listen on the two new **language-name clips** (`Tiếng Việt` / `English`, E23 / Y31–Y34 — they do not exist yet); a listen on the 8 new Vietnamese rime-prefix clips, and 24 English `short` clips still over the 700 ms target |
